...
export const Inner = observer(() => {
    ...
    return (
        <>
            <button disabled={leagueStore.currentTour > (leagueStore.getTeams().length - 1) * 2} onClick={playTour}>play tour</button>
            <button disabled={leagueStore.status !== 'waiting'} onClick={() => leagueStore.startLeague()}>start league
            </button>
            <button disabled={leagueStore.currentTour <= (leagueStore.getTeams().length - 1) * 2} onClick={() => leagueStore.endLeague()}>end league
            </button>
