...
class LeagueStore {
    ...
    finishTour(tourResult: Match2[]) {
        ...
        tourResult.forEach((match, index) => {
            ...
            const homeTeam = teams.find(team => team.name === home.name);
            const awayTeam = teams.find(team => team.name === away.name);
            if (matchStatus === 'H') {
        ...
        if (this.currentTour === this.calendar.length) {
            this.endLeague()
        } else {
            this.currentTour++;
        }
    ...
    generateSchedule() {
        this.calendar = generateRoundRobinMatches(this.teams);
    }
